                ...
                button {
                    ...
                    onclick: move |_| {
                        ...
                        if !email_val.contains('@') || !email_val.contains('.') {
                            status_type.set("error".to_string());
                            submit_status
                                .set(Some("❌ Please enter a valid email address.".to_string()));
                            return;
                        }
                        ...
                        spawn(async move {
                            ...
                            let submission = NewContactSubmission {
                                ...
                                metadata: Some(
                                    json!(
                                        { "source" : "website", "timestamp" : chrono::Utc::now()
                                        .to_rfc3339() }
                                    ),
                                ),
                            };
                            match submit_contact_form(submission).await {
                                ...
                                Err(error) => {
                                    ...
                                    submit_status
                                        .set(Some(format!("{} Error details: {}", error_msg, error)));
                                }
                            }

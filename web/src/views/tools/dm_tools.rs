...
pub fn DMTools() -> Element {
    let dice_roll = use_signal(|| (0, 0));
    let npc_name = use_signal(|| String::new());
    let encounter_difficulty = use_signal(|| "medium".to_string());
    let generated_encounter = use_signal(|| Vec::new());
    let treasure_roll = use_signal(|| Vec::new());
    let weather_conditions = use_signal(|| String::new());
    let random_event = use_signal(|| String::new());

    // Sample data
    let name_generator = use_ref(|| vec![
    ...
    let generate_encounter = {
        let generated_encounter = generated_encounter.clone();
    ...
    let generate_treasure = {
        let treasure_roll = treasure_roll.clone();
    ...
    let generate_event = move |_| {
                        button {
                            class: "p-3 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 transition-colors",
                            onclick: |_| roll_dice(4),
                            "d4"
                        }
                        button {
                            class: "p-3 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 transition-colors",
                            onclick: |_| roll_dice(6),
                            "d6"
                        }
                        button {
                            class: "p-3 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 transition-colors",
                            onclick: |_| roll_dice(8),
                            "d8"
                        }
                        button {
                            class: "p-3 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 transition-colors",
                            onclick: |_| roll_dice(10),
                            "d10"
                        }
                        button {
                            class: "p-3 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 transition-colors",
                            onclick: |_| roll_dice(12),
                            "d12"
                        }
                        button {
                            class: "p-3 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 transition-colors",
                            onclick: |_| roll_dice(20),
                            "d20"
                        }
                        }
                        button {
                            class: "p-3 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 transition-colors",
                            onclick: move |_| roll_dice(10),
                            "d10"
                        }
                        button {
                            class: "p-3 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 transition-colors",
                            onclick: move |_| roll_dice(12),
                            "d12"
                        }
                        button {
                            class: "p-3 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 transition-colors",
                            onclick: move |_| roll_dice(20),
                            "d20"
                        }
                        for (race , _) in name_generator.read().iter() {
                            let race = race.to_string();
                            let generate_npc = generate_npc.clone();
                            button {
                                class: "p-3 bg-green-100 text-green-800 rounded hover:bg-green-200 transition-colors",
                                onclick: move |_| generate_npc(&race),
                                {race}
                            }
                        }
                    }
                ...
                div { class: "bg-white rounded-lg shadow p-6",
                    ...
                    div { class: "grid grid-cols-3 gap-3 mb-4",
                        for (race , _) in &name_generator {
                            button {
                                class: "p-3 bg-green-100 text-green-800 rounded hover:bg-green-200 transition-colors",
                                onclick: move |_| generate_npc(race),
                                {race}
                            onchange: |e| encounter_difficulty.set(e.value().clone()),
                        }
                    }
                    if !npc_name.read().is_empty() {
                        div { class: "mt-4 p-3 bg-green-50 rounded border border-green-200",
                            p { class: "text-xl font-medium text-center", "{npc_name}" }
                        }
                    }
                }
                ...
                div { class: "bg-white rounded-lg shadow p-6",
                    h2 { class: "text-xl font-semibold mb-4 text-gray-800 border-b pb-2",
                            ...
                            onchange: move |e| encounter_difficulty.set(e.value().clone()),
                    ...
                    button {
                        ...
                        button {
                            ...
                            onclick: |_| generate_treasure("common"),
                            "Common"
                        }
                        button {
                            class: "p-3 bg-yellow-100 text-yellow-800 rounded hover:bg-yellow-200 transition-colors",
                            onclick: |_| generate_treasure("uncommon"),
                            "Uncommon"
                        }
                        button {
                            class: "p-3 bg-yellow-100 text-yellow-800 rounded hover:bg-yellow-200 transition-colors",
                            onclick: |_| generate_treasure("rare"),
                            "Rare"
                        }
                        "Treasure Generator"
                    }
                ...
                div { class: "bg-white rounded-lg shadow p-6",
                    ...
                    if !weather_conditions.read().is_empty() {
                        div { class: "p-3 bg-blue-50 rounded border border-blue-200 text-center",
                            p { class: "text-lg", "{weather_conditions.read().clone()}" }
                        }
                    }
                }
                ...
                div { class: "bg-white rounded-lg shadow p-6",
                    ...
                    if !random_event.read().is_empty() {
                        div { class: "p-3 bg-purple-50 rounded border border-purple-200",
                            p { class: "text-lg", "{random_event.read().clone()}" }
                        }
                    }
                }
            }
